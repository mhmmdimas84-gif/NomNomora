...
<div class="flex flex-col gap-6 max-w-3xl">
    <div class="flex flex-col gap-1">
        <h1 class="text-3xl font-extrabold text-gray-900">Tambah Testimonial</h1>
        <p class="text-gray-500">Tambahkan testimoni kepuasan pelanggan NomNomora secara manual.</p>
    </div>

    <div class="bg-white border border-gray-100 rounded-2xl shadow-sm p-8">
        <form action="{{ route('admin.testimonials.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col gap-2" x-data="{ photoUrl: null }">
            ...
            <div class="flex flex-col gap-1.5">
                ...
                <textarea id="message" name="message" rows="4" required
                          ...
                          placeholder="Tulis ulasan pelanggan mengenai produk dan pelayanan NomNomora...">{{ old('message') }}</textarea>
                @error('message')
                    <span class="text-xs text-red-500 font-semibold mt-0.5">{{ $message }}</span>
                @enderror
            </div>
