...
<div class="flex flex-col gap-6 max-w-2xl">
    <div class="flex flex-col gap-1">
        <h1 class="text-3xl font-extrabold text-gray-900">Ubah Kategori</h1>
        <p class="text-gray-500">Edit detail kategori menu makanan & minuman NomNomora.</p>
    </div>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="flex flex-col gap-6">
            ...
            <div class="flex flex-col gap-2">
                ...
                <textarea id="description" name="description" rows="4"
                          ...
                          placeholder="Jelaskan kategori ini secara singkat...">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <span class="text-xs text-red-500 font-semibold">{{ $message }}</span>
                @enderror
            </div>
