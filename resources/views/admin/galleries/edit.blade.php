...
<div class="flex flex-col gap-6 max-w-3xl">
    <div class="flex flex-col gap-1">
        <h1 class="text-3xl font-extrabold text-gray-900">Ubah Foto Galeri</h1>
        <p class="text-gray-500">Edit detail foto dokumentasi NomNomora.</p>
    </div>

    <div class="bg-white border border-gray-100 rounded-2xl shadow-sm p-8">
        <form action="{{ route('admin.galleries.update', $gallery) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col gap-5">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <textarea id="description" name="description" rows="3"
                                  ...
                                  placeholder="Keterangan singkat mengenai foto ini...">{{ old('description', $gallery->description) }}</textarea>
                        @error('description')
                            <span class="text-xs text-red-500 font-semibold mt-0.5">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="flex flex-col gap-2" x-data="{ imageUrl: '{{ Str::startsWith($gallery->image, 'gallery_') ? '' : asset('storage/' . $gallery->image) }}' }">
